...
async function getAllServices(args) {
  try {
    ...
    const objectIdFields = ["category", "sub_category"];
    ...
    for (let key in filters) {
      if (
        ...
        if (key === "search") {
          ...
          // Create OR conditions for each word across name and description
          filterConditions.$or = [
        ...
        } else if (objectIdFields.includes(key) && mongoose.Types.ObjectId.isValid(filters[key])) {
...
async function createService(args) {
  try {
    ...
    const service = await ServiceModel.create(args);
    const updateSubCategory = await SubCategoryModel.findOneAndUpdate(
      { _id: args.sub_category },
      { $push: { services: service._id } }
    );
    if (!updateSubCategory) throw new Error("Sub-Category not found");
    return {
      success: true,
...
async function updateService(args) {
  try {
    const service = await ServiceModel.findByIdAndUpdate(args._id, args.input, {
      new: true,
    });
    if (!service) throw new Error("Service not found");
    return {
